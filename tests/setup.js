 ...
 */
function generateValidTestData(_schema, count = 1) {
	const results = [];
 ...
 */
function generateInvalidTestData(_schema, count = 1) {
	const results = [];
	for (let i = 0; i < count; i++) {
		const data = {
...
expect.extend({
	toValidateSuccessfully(schema, data) {
		...
		if (pass) {
			...
			return {
				message: () =>
					`Expected schema to accept data: ${JSON.stringify(data)}\nErrors: ${JSON.stringify(result.error?.errors)}`,
				pass: false,
			};
		}
	},
	toRejectWithError(schema, data, expectedError) {
		...
		if (pass && expectedError) {
			const hasExpectedError = result.error?.errors.some(
				(err) => err.message.includes(expectedError) || err.code === expectedError,
			);
			if (!hasExpectedError) {
				return {
					message: () =>
						`Expected schema to reject with error "${expectedError}", but got: ${JSON.stringify(result.error?.errors)}`,
					pass: false,
				};
			}
		}
