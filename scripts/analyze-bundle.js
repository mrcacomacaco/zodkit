...
function analyzeDirectory(dir, baseDir = dir) {
	...
	results.largestFiles = results.files.sort((a, b) => b.size - a.size).slice(0, 10);
...
function main() {
	...
	console.log(
		`  Compression ratio: ${((1 - distAnalysis.totalGzipped / distAnalysis.totalSize) * 100).toFixed(1)}%`,
	);
	...
	for (const file of distAnalysis.largestFiles.slice(0, 5)) {
		const sizeColor =
			file.size > 50000 ? colors.red : file.size > 20000 ? colors.yellow : colors.green;
		console.log(
			`  ${sizeColor}${file.path.padEnd(50)}${colors.reset} ${formatBytes(file.size).padStart(10)} → ${formatBytes(file.gzipped).padStart(10)} (${file.ratio}% smaller)`,
		);
	}
	...
	try {
		...
		const packOutput = execSync('npm pack --dry-run 2>&1 | grep -E "npm notice"', {
			encoding: 'utf8',
		});
	...
	for (const dep of depAnalysis.slice(0, 5)) {
		...
		const sizeColor =
			dep.size > 1000000 ? colors.red : dep.size > 500000 ? colors.yellow : colors.green;
		console.log(
			`  ${sizeColor}${dep.name.padEnd(30)}${colors.reset} ${formatBytes(dep.size).padStart(10)} (${percentage}% of deps)`,
		);
	}
	...
	const checkLimit = (name, value, limit) => {
		...
		console.log(
			`  ${name.padEnd(20)} ${formatBytes(value).padStart(10)} / ${formatBytes(limit).padStart(10)} ${status} (${color}${percentage}%${colors.reset})`,
		);
	};
	...
	// Final status
	const allPassed =
		distAnalysis.totalSize <= limits.dist && distAnalysis.totalGzipped <= limits.gzipped;
