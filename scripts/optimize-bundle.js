...
function generateBundleReport() {
  ...
  console.log('\n🎯 Size Targets:');
  console.log(`Total:          ${formatBytes(targets.total)} ${distSize <= targets.total ? '✅' : '❌'}`);
  console.log(`CLI:            ${formatBytes(targets.cli)} ${cliSize <= targets.cli ? '✅' : '❌'}`);
  console.log(`Core:           ${formatBytes(targets.core)} ${coreSize <= targets.core ? '✅' : '❌'}`);
