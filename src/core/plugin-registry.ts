...
import type { Plugin, PluginConfig } from './plugin-system';
...
export class PluginRegistry extends EventEmitter {
   ...
   */
  async searchPlugins(query: string, options: PluginSearchOptions = {}): Promise<PluginPackageInfo[]> {
  ...
  private async loadPluginInfo(packageName: string): Promise<PluginPackageInfo | null> {
    try {
      const packagePath = path.join(this.workingDirectory, 'node_modules', packageName, 'package.json');
  ...
  private loadInstalledPlugins(): void {
    ...
    this.scanLocalPlugins().then(plugins => {
      for (const plugin of plugins) {
