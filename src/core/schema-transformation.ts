...
export class SchemaTransformer {
	...
	private createMigrationPlan(_schema: z.ZodTypeAny): MigrationPlan {
		return {
			version: '2.0.0',
			steps: [
				{
	...
	private hasDuplicateLogic(_schema: z.ZodTypeAny): boolean {
	...
	private zodToTypeScript(_schema: z.ZodTypeAny): string {
		// Convert Zod to TypeScript
		return `type Schema = z.infer<typeof schema>`;
	}

	private zodToJsonSchema(_schema: z.ZodTypeAny): object {
...
export class SchemaBridge extends SchemaTransformer {
	async convert(input: any, _target: string): Promise<string> {
...
export class SchemaMigration extends SchemaTransformer {
	async migrateToVersion(schema: z.ZodTypeAny, version: string): Promise<z.ZodTypeAny> {
		const result = await this.transform(schema, {
			type: 'migrate',
...
export {
	...
	type TransformationOptions as CompositionConfig,
	type TransformationType as CompositionOperation,
	type TransformationOptions as CompositionStrategy,
};
