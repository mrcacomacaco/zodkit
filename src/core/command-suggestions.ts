...
export class CommandSuggestionEngine {
  ...
  private getInputSuggestions(input: string): CommandSuggestion[] {
    ...
    const commandMatches = [
      { pattern: /check|valid|analyz/, command: 'check', description: 'Analyze schemas for issues' },
      { pattern: /scaffold|generat|convert/, command: 'scaffold', description: 'Generate Zod from TypeScript' },
      { pattern: /hint|suggest|optim/, command: 'hint', description: 'Get optimization suggestions' },
      { pattern: /test|valid/, command: 'test', description: 'Run schema tests' },
      { pattern: /perf|speed|slow/, command: 'perf', description: 'Profile schema performance' },
      { pattern: /fix|repair/, command: 'fix', description: 'Auto-fix schema issues' },
      { pattern: /mock|fake|data/, command: 'mock', description: 'Generate mock data' },
      { pattern: /ui|dash|interface/, command: 'ui', description: 'Launch unified dashboard' }
    ];
  ...
  private detectSchemas(cwd: string): boolean {
    ...
    return existsSync(resolve(cwd, 'src/schemas')) ||
  ...
  private detectTests(cwd: string): boolean {
    return existsSync(resolve(cwd, 'test')) ||
  ...
  private detectConfig(cwd: string): boolean {
    return existsSync(resolve(cwd, 'zodkit.config.js')) ||
