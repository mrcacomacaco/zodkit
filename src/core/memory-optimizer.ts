...
export class MemoryMonitor extends EventEmitter {
	...
	private collectStats(): MemoryStats {
		...
		if (heapUsedMB > this.thresholds.criticalMB || heapUtilization > 0.95) {
			...
			heapUsedMB > this.thresholds.warningMB ||
			heapUtilization > this.thresholds.maxHeapUtilization
		) {
			pressureLevel = 'high';
...
export class StreamingProcessor extends EventEmitter {
	...
	private handleMemoryPressure(stats: MemoryStats): void {
		console.warn(
			`${pc.yellow('⚠️  Memory pressure detected')}: ${Math.round(stats.heapUsed / 1024 / 1024)}MB used`,
		);
	...
	private handleCriticalMemoryPressure(stats: MemoryStats): void {
		console.error(
			`${pc.red('🚨 Critical memory pressure')}: ${Math.round(stats.heapUsed / 1024 / 1024)}MB used`,
		);
						...
						transform: async (chunk: Buffer, _encoding, callback) => {
							try {
								...
								if (lines.length > 0) {
									...
									const result = await processor(chunkContent, {
										filePath,
										chunkIndex,
									});
...
export class MemoryOptimizer extends EventEmitter {
	...
	private optimizeMemory(stats: MemoryStats): void {
		console.log(
			`${pc.cyan('🔧 Optimizing memory')}: ${Math.round(stats.heapUsed / 1024 / 1024)}MB used`,
		);
	...
	private forceOptimization(stats: MemoryStats): void {
		console.warn(
			`${pc.red('🚨 Force optimizing memory')}: ${Math.round(stats.heapUsed / 1024 / 1024)}MB used`,
		);
