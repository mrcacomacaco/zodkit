		...
		switch (operation.type) {
			case 'insert':
				if (operation.content) {
					const newLine =
						currentLine.slice(0, column) + operation.content + currentLine.slice(column);
					lines[line] = newLine;
				}
				break;
			...
			case 'replace':
				if (operation.content && operation.previousContent) {
					...
					const newLine =
						...
						operation.content +
						currentLine.slice(column + replaceLength);
					lines[line] = newLine;
				}
				break;
		}
...
export class CollaborationEngine extends EventEmitter {
	...
	async addComment(
		...
		comment: Omit<
			...
			'id' | 'createdAt' | 'updatedAt' | 'replies' | 'reactions' | 'isResolved'
		>,
	...
	getUserSessions(userId: string): CollaborationSession[] {
		return Array.from(this.sessions.values()).filter((s) => s.isActive && s.users.has(userId));
	}
	...
	private operationsOverlap(op1: CollaborationOperation, op2: CollaborationOperation): boolean {
		...
		return (
			Math.abs(op1.position.line - op2.position.line) <= 1 &&
	...
	private cleanupInactiveSessions(): void {
		...
		for (const [_sessionId, session] of this.sessions) {
			...
			const allUsersInactive = Array.from(session.users.values()).every(
				(user) => now - user.lastSeen > inactiveThreshold,
			);
