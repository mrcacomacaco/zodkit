...
export class HotReloadManager extends EventEmitter {
  ...
  private async buildDependencyGraph(): Promise<void> {
    ...
    const schemaFiles = this.infrastructure
      ? await this.infrastructure.discoverSchemas()
      : [];
    ...
    this.logger.info(`Built dependency graph with ${this.dependencyGraph.size} nodes in ${buildTime}ms`);
  }
  ...
  private async handleFileChange(filePath: string, changeType: 'change' | 'add' | 'unlink'): Promise<void> {
    const absolutePath = resolve(filePath);
  ...
  private async processFileChange(filePath: string, changeType: 'change' | 'add' | 'unlink'): Promise<void> {
    const startTime = Date.now();
  ...
  private async processBatchedReloads(): Promise<void> {
    ...
    try {
      const filesToReload = Array.from(this.reloadQueue.keys())
        .slice(0, this.config.performance.updateBatchSize);
  ...
  private async reloadFile(filePath: string): Promise<void> {
    ...
    try {
      ...
      this.logger.debug(`Reloaded ${schemas.length} schemas from ${relative(process.cwd(), filePath)}`);
