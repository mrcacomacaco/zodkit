...
export class CircuitBreaker extends EventEmitter {
  ...
  private open(): void {
    ...
    console.warn(`${pc.yellow('⚡ Circuit breaker OPENED')} for ${this.operation} (${this.failures} failures)`);
  }
...
export class ErrorRecoveryManager extends EventEmitter {
  ...
  async handleError(
    ...
    console.error(`${pc.gray('Context')}: ${context.operation}${context.file ? ` in ${context.file}` : ''}`);
   ...
   */
  private getContextSpecificStrategies(error: Error, context: ErrorContext): RecoveryAction[] {
  ...
  getErrorStats(): {
    ...
    const resolvedErrors = Array.from(this.errorReports.values())
      .filter(report => report.resolved).length;

    const circuitBreakerStates = Object.fromEntries(
      Array.from(this.circuitBreakers.entries())
        .map(([operation, breaker]) => [operation, breaker.getState()])
    );
...
export class GracefulDegradation {
  ...
  getStatus(): {
    ...
    recentDegradations: Array<{ feature: string; timestamp: number; reason: string }>;
  } {
