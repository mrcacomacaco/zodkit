...
function displayTextMap(map: unknown, focusSchema?: string): void {
	...
	console.log(
		pc.cyan('Circular Dependencies:'),
		mapData.metadata.circularDependencies?.length ?? 0,
	);
	...
	const orphaned = mapData.schemas.filter(
		(s) => !mapData.relationships.some((r) => r.to === s.name || r.from === s.name),
	);
...
function displaySchemaDetails(schema: unknown, map: unknown): void {
	...
	if (dependencies.length > 0) {
		...
		dependencies.forEach((dep) => {
			...
			console.log(
				`  ├── ${pc.cyan(dep.to)} ${pc.gray(`(${dep.type})`)} - ${target?.file ?? 'unknown'}`,
			);
		});
	}
	...
	if (dependents.length > 0) {
		...
		dependents.forEach((dep) => {
			...
			console.log(
				`  ├── ${pc.cyan(dep.from)} ${pc.gray(`(${dep.type})`)} - ${source?.file ?? 'unknown'}`,
			);
		});
	}
...
function displayMapOverview(map: unknown): void {
	...
	const schemasByType = mapData.schemas.reduce(
		(acc: Record<string, typeof mapData.schemas>, schema) => {
			acc[schema.type] ??= [];
	...
	Object.entries(schemasByType).forEach(([type, schemas]) => {
		...
		schemas.slice(0, 5).forEach((s) => {
			console.log(
				`    ├── ${pc.cyan(s.name)} ${pc.gray(`(complexity: ${s.complexity.toFixed(1)})`)}`,
			);
		});
		if (schemas.length > 5) {
	...
	const connectionCounts = mapData.schemas
		.map((schema) => ({
			name: schema.name,
			connections: mapData.relationships.filter(
				(r) => r.from === schema.name || r.to === schema.name,
			).length,
		}))
	...
	if (connectionCounts.length > 0) {
		...
		connectionCounts.slice(0, 10).forEach((s) => {
			console.log(
				`  ${pc.cyan(s.name)}: ${s.connections} connection${s.connections === 1 ? '' : 's'}`,
			);
		});
	}

	// Show relationship types
	const relationshipTypes = mapData.relationships.reduce(
		(acc: Record<string, number>, rel) => {
