...
async function initializeProject(
  ...
  fs.writeFileSync(
    path.join(projectDir, 'src/schemas/example.schema.ts'),
    exampleSchema,
    'utf8'
  );
...
paths:
${schemas.map(schema => `  /${schema.name.toLowerCase()}:
    get:
      ...
      responses:
        '200':
          ...
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/${schema.name}'`).join('\n')}

components:
  schemas:
${schemas.map(schema => `    ${schema.name}:
      type: object
      description: ${schema.name} schema`).join('\n')}
`;
}

function generateGraphQLContract(schemas: any[]): string {
  return schemas.map(schema => `type ${schema.name} {
  id: ID!
...
${schemas.map(schema => `message ${schema.name} {
  string id = 1;
  // Add fields based on schema
}`).join('\n\n')}
`;
...
export function registerSetupCommand(program: Command): void {
  ...
  program
    ...
    .action((name, options, cmd) =>
      setupCommand(name, { ...options, mode: 'init' }, cmd)
    );
  ...
  program
    ...
    .action((output, options, cmd) =>
      setupCommand(output, { ...options, mode: 'contract' }, cmd)
    );
}
