...
Force: ${options.force ? 'enabled' : 'disabled'}`,
      data: { operation: 'fix', mode, options: { dryRun: options.dryRun, safeOnly: options.safeOnly } }
    });
    ...
    if (!isQuiet && !isJsonMode) {
      logger.info(`Found ${pc.cyan(targetSchemas.length)} schema${targetSchemas.length !== 1 ? 's' : ''} to analyze`);
    }
    ...
    if (allFixes.length === 0) {
      if (isJsonMode) {
        ...
        if (totalIssues > 0) {
          console.log(`   ${totalIssues} issue${totalIssues !== 1 ? 's' : ''} require manual attention`);
