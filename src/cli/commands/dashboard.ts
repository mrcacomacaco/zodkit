...
export async function dashboardCommand(
	...
	try {
		...
		if (schemas.length === 0) {
			...
			console.log(
				pc.green(`✅ Found ${schemas.length} Zod schema${schemas.length !== 1 ? 's' : ''}`),
			);
			console.log();
		}
 ...
 */
export async function simpleDashboardCommand(_options: DashboardOptions = {}): Promise<void> {
	console.clear();
	...
	try {
		...
		console.log(
			`  • ${pc.red('Error reading project')}: ${error instanceof Error ? error.message : String(error)}`,
		);
	}
