...
async function handleCreateSession(
	...
	if (options.conflictResolution !== undefined)
		sessionConfig.conflictResolution = options.conflictResolution;
	if (options.requireApproval !== undefined)
		sessionConfig.requireApproval = options.requireApproval;
	...
	if (options.enableVersioning !== undefined)
		sessionConfig.enableVersioning = options.enableVersioning;
	...
	if (isJsonMode) {
		...
		console.log(
			`  Web URL: ${pc.cyan(`http://localhost:${options.port ?? 3000}/session/${session.id}`)}`,
		);
...
function handleListSessions(
	...
	if (isJsonMode) {
		...
		sessions.forEach((session, index) => {
			...
			console.log(
				`   Mode: ${session.config.mode} | Users: ${session.users.size} | Files: ${session.files.size}`,
			);
...
function handleSessionStatus(
	...
	if (isJsonMode) {
		...
		console.log(
			`${pc.cyan('Status:')} ${session.isActive ? pc.green('Active') : pc.red('Inactive')}`,
		);
		console.log(`${pc.cyan('Mode:')} ${session.config.mode}`);
		...
		console.log(
			`  Conflicts: ${session.conflicts.length} (${session.conflicts.filter((c) => !c.resolvedAt).length} unresolved)`,
		);
...
async function handleResolveConflicts(
	...
	if (isJsonMode) {
		...
		if (remainingConflicts.length > 0) {
			console.log(
				pc.yellow(`⚠️  ${remainingConflicts.length} conflicts require manual resolution:`),
			);
			...
			remainingConflicts.forEach((conflict, index) => {
				...
				console.log(
					`   Command: ${pc.cyan(`zodkit collaborate resolve --session-id ${session.id} --conflict-id ${conflict.id}`)}`,
				);
			});
		}
	}
...
function handleWatchSession(
	...
	engine.on('operationApplied', ({ session: updatedSession, operation }: OperationEvent) => {
		...
		if (!isJsonMode) {
			...
			console.log(
				`${pc.green('📝')} ${user?.name ?? 'Unknown'} ${operation.type} in ${operation.file}`,
			);
		}
	});
...
function handleWebSocketMessage(_ws: WebSocket, message: unknown): void {
