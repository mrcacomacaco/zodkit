...
import { APIInspector, DataAnalyzer } from '../../core/analysis';
...
export async function generateCommand(options: GenerateOptions): Promise<void> {
	try {
		...
		console.error(
			pc.red('❌ Generate command failed:'),
...
class ZodSchemaGenerator implements LegacySchemaGenerator {
	generateFromJson(data: unknown, _options: GenerateOptions): string {
		const schemaName = 'GeneratedSchema';
		const schema = this.inferZodSchemaFromValue(data, _options.strict ?? false);
		...
		if (components && typeof components === 'object') {
			...
			if (schemasObj && typeof schemasObj === 'object') {
				for (const [name, schema] of Object.entries(schemasObj)) {
					const zodSchema = this.convertOpenAPISchemaToZod(
...
`;
	...
	private convertOpenAPISchemaToZod(schema: Record<string, unknown>, strict: boolean): string {
		...
		switch (schema.type) {
			...
			case 'boolean':
				return 'z.boolean()';
			case 'array': {
				const itemSchema = this.convertOpenAPISchemaToZod(
	...
	private convertOpenAPIObjectToZod(schema: Record<string, unknown>, strict: boolean): string {
		...
		for (const [key, propSchema] of Object.entries(schema.properties)) {
			const propZodSchema = this.convertOpenAPISchemaToZod(
...
async function generateFromAdvancedSources(
	...
	if (options.fromJson) {
		...
		if (!options.overwrite && existsSync(outputFile)) {
			...
			console.log(
				pc.gray(`   Complexity: ${schema.complexity}, Patterns: ${schema.patterns.join(', ')}`),
			);
	...
	if (options.fromUrl) {
		...
		const responses = await (services.apiInspector as any).inspectAPI(options.fromUrl, {
			methods: ['GET', 'POST'],
			sampleCount: parseInt(options.samples || '3', 10),
			timeout: 10000,
		...
		for (const [index, response] of responses.entries()) {
			...
			const schema = await (services.coreGenerator as any).generateFromAnalysis(
				analysis,
				apiGenOptions,
			);
			...
			const outputFile = join(
				outputDir,
				`${options.name || 'api'}-${response.method.toLowerCase()}-${index}.schema.ts`,
			);
			const finalCode = `${schema.zodCode}\n\n${schema.typeCode}`;
	...
	if (options.fromDatabase && options.connection) {
		...
		if (options.fromDatabase === 'all') {
			...
			for (const tableSchema of tableSchemas) {
				...
				const schema = await (services.coreGenerator as any).generateFromDatabaseSchema(
					tableSchema,
					dbGenOptions,
				);
			...
			const tableSchema = await (services.dbConnector as any).analyzeTable(
				options.connection,
	...
	if (options.learn) {
		...
		for (const [_index, pattern] of extractedPatterns.entries()) {
			...
			const schema = await (services.coreGenerator as any).generateFromPattern(
				pattern,
				generationOptions,
			);
			...
			if (!options.overwrite && existsSync(outputFile)) {
				...
				console.log(
					pc.green(
						`🎯 Learned pattern "${pattern.name}" (${pattern.occurrences} occurrences, ${Math.round(pattern.confidence * 100)}% confidence)`,
					),
				);
			}
		}
...
async function generateFromJsonFiles(
...
async function generateFromTypeScriptFiles(
	generator: ZodSchemaGenerator,
	options: GenerateOptions,
): Promise<string[]> {
	const inputPath = resolve(options.input!);
...
function generateFromOpenAPISpec(
	generator: ZodSchemaGenerator,
	options: GenerateOptions,
): string[] {
	const inputPath = resolve(options.input!);
