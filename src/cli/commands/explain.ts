...
import { SchemaCache, SchemaDiscovery } from '../../core/infrastructure';
...
export async function explainCommand(
	...
	try {
		...
		if (schemaName) {
			targetSchemas = schemas.filter((s) => s.name === schemaName || s.exportName === schemaName);
			...
			if (isJsonMode) {
				...
				schemas.forEach((s) => {
					console.log(
						`  ${pc.cyan(s.exportName ?? s.name)} ${pc.gray(`(${s.schemaType})`)} - ${s.filePath}`,
					);
				});
		...
		for (const schema of targetSchemas) {
			const explanation = await generateExplanation(schema, options, schemas, analyzer);
...
async function generateExplanation(
	...
	// Analyze complexity
	const complexityMetrics = await analyzer.analyze(schema as any, {
		mode: 'complexity',
	});
...
function displayExplanations(explanations: SchemaExplanation[]): void {
	explanations.forEach((exp, index) => {
		...
		const complexityColor =
			exp.complexity.level === 'low'
				? pc.green
				: exp.complexity.level === 'medium'
					? pc.yellow
					: exp.complexity.level === 'high'
						? pc.red
						: pc.bgRed;
		console.log(
			`${pc.bold('Complexity:')} ${complexityColor(`${exp.complexity.level} (${exp.complexity.score.toFixed(1)})`)}`,
		);
		...
		if (exp.fields && exp.fields.length > 0) {
			...
			exp.fields.forEach((field) => {
				...
				console.log(
					`  ${pc.cyan(field.name)}${required} ${pc.gray(`(${field.type})`)} ${field.validations.join(', ')}`,
				);
		...
		if (exp.usage && exp.usage.length > 0) {
			console.log(`\n${pc.bold('Usage:')}`);
			const usageByType = exp.usage?.reduce(
				(acc, u) => {
					acc[u.type] ??= [];
					acc[u.type]?.push(u);
					return acc;
...
function generateExamples(schema: unknown): {
	valid: unknown[];
	invalid: unknown[];
} {
