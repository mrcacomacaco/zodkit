...
export async function docsCommand(options: DocsOptions): Promise<void> {
	try {
		...
		console.error(
			pc.red('❌ Documentation generation failed:'),
...
async function parseSchemaFile(filePath: string): Promise<SchemaDoc[]> {
	...
	const schemaRegex =
		/export\s+const\s+(\w+(?:Schema|Validator|Model))\s*=\s*z\.([\s\S]*?)(?=\n\n|\nexport|\n\/\/|\n\*|$)/g;
...
function extractConstraints(definition: string): string[] {
	...
	const constraintPatterns = [
		{
			pattern: /\.min\((\d+)\)/,
			format: (match: RegExpMatchArray) => `minimum: ${match[1]}`,
		},
		{
			pattern: /\.max\((\d+)\)/,
			format: (match: RegExpMatchArray) => `maximum: ${match[1]}`,
		},
		{
			pattern: /\.length\((\d+)\)/,
			format: (match: RegExpMatchArray) => `exact length: ${match[1]}`,
		},
...
async function generateJsonDocs(
	schemas: SchemaDoc[],
	outputPath: string,
