...
async function showSyncStatus(
	...
	try {
		...
		console.log(
			`   ${pc.gray('Conflicts:')} ${conflicts.length > 0 ? pc.red(conflicts.length) : pc.green('0')}`,
		);
		...
		if (conflicts.length > 0) {
			...
			console.log(pc.blue('\nResolve conflicts:'));
			console.log(
				`  ${pc.gray('$')} zodkit sync --conflicts interactive    # Interactive resolution`,
			);
			console.log(
				`  ${pc.gray('$')} zodkit refactor --suggest              # Get refactoring suggestions`,
			);
		}
...
async function startWatchMode(
	...
	try {
		...
		console.log(
			pc.red(`❌ Watch mode failed: ${error instanceof Error ? error.message : String(error)}`),
		);
	}
}
	...
	try {
		...
		if (result.discovered === 0 && result.updated === 0 && result.removed === 0) {
			...
			if (result.discovered > 0)
				console.log(`   ${pc.cyan('•')} Discovered: ${result.discovered} schemas`);
			if (result.updated > 0)
				console.log(`   ${pc.yellow('•')} Updated: ${result.updated} schemas`);
