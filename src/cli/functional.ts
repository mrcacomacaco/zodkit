...
program
  ...
  .action(async (projectName, options) => {
    try {
      ...
      console.error(pc.red('❌ Init failed:'), error instanceof Error ? error.message : String(error));
      process.exit(1);
    }
  });
...
program
  ...
  .action(async (schema, options) => {
    try {
      ...
      console.error(pc.red('❌ Fix failed:'), error instanceof Error ? error.message : String(error));
      process.exit(1);
    }
  });
...
async function performInit(projectName?: string, _options?: any) {
  ...
  try {
    await writeFile(
      join(process.cwd(), 'zodkit.config.json'),
